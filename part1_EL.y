...
%{
    ...
    typedef struct ast_node {
        char             token;   / label – operator / nt‑name */
        struct ast_node  left;    / first  child  */
        struct ast_node  right;   / second child / siblings   */
    } *node;
    ...
    static int printLevel = 0;     /* indent while printing tree */
%}

/* ------------ union -------------- */
%union{
    node  n;          /* pointer to AST node                    */
    char str;        / IDENT / literal string                 */
...
/* whole program – print AST then exit */
