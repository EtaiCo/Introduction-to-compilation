...
assign_state
  ...
  | IDENT '[' expression ']' ASSIGN expression ';'
    ...
    | MULTI IDENT ASSIGN expression ';'
    {
        ...
        if (!v || !isPointerType(v->returnType)) {
            yyerror("Semantic Error: Left-hand side must be a pointer.");
            YYABORT;
        }

        char* rhsType = inferExprType($4);
        if (strcasecmp(v->returnType, "intptr") == 0 && !isInt(rhsType) ||
            strcasecmp(v->returnType, "realptr") == 0 && !isReal(rhsType) ||
            strcasecmp(v->returnType, "charptr") == 0 && !isChar(rhsType)) {
            yyerror("Semantic Error: Dereferenced pointer type mismatch.");
            YYABORT;
        }

        $$ = mknode("deref_assign", mknode($2, NULL, NULL), $4);
    }
