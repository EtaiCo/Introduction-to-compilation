...
function :
    DEF IDENT '(' params ')' ':' RETURNS type var scope_marker T_BEGIN statements END
    {
        ...
        while (curParam && paramCount < MAX_PARAMS) {
            if (strcmp(curParam->token, "PARAMS") == 0) {
                ...
                if (single && strcmp(single->token, "PARAM") == 0 && single->left) {
                    paramTypes[paramCount++] = single->left->token;  // this is the type
                }
        ...
        if (insertSymbol($2, FUNC, $8->token, paramCount, "global", paramTypes)) {
...
func_call :
    ...
    // TODO (Rule 8): Check parameter types here if type inference is implemented
...
char* inferExprType(node* expr) {
    ...
    if (expr->token[0] == '\'' && strlen(expr->token) == 3) return "char"; // char literal: 'c'
    if (expr->token[0] == '"') return "string"; // crude check for string
    // fallback for identifiers
    Symbol* sym = lookupSymbol(expr->token);
    if (sym && sym->type == VAR) return sym->returnType;
    return "unknown";
}
