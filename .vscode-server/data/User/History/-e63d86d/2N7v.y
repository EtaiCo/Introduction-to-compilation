...
%type <nodePtr> param var dec_list dec type literal idents_list ident_entry
%type <nodePtr> statements state assign_state
%type <nodePtr> if_state while_state do_while_state alt_body elif_list_opt elif_list else_opt
...
ident_entry :
    ...
    | IDENT '[' INT_LIT ']'        { 
        ...
        $$ = mknode("ARRAY_DECL", mknode("IDENT", mknode($1, NULL, NULL), NULL), mknode(buf, NULL, NULL)); 
    }
...
idents_list
    : ident_entry                         { $$ = $1; }
    | ident_entry ',' idents_list         { $$ = mknode("ID_LIST",$1,$3); }
    ;
...
/* -----------------------------  IF / ELIF / ELSE ------------------------*/
/* ---------- helper: a body can be a block OR one statement -------------*/


        alt_body
        : state          { $$ = $1; }   /* single statement */
        | bl_state       { $$ = $1; }   /* begin … end block */
        ;


/* ---------- IF / ELIF / ELSE ------------------------------------------*/
if_state
        : IF   expression ':' alt_body                 /* plain if */
            elif_list_opt else_opt
              { $$ = mknode("if_full",
                           mknode("cond" ,$2 , NULL),
                           mknode("then" ,$4 ,
                           mknode("elifs",$5 ,
...
elif_list
        : ELIF expression ':' alt_body
              { $$ = mknode("elif",
                           mknode("cond",$2,NULL),
                           mknode("then",$4,NULL)); }
        | ELIF expression ':' alt_body elif_list
              { $$ = mknode("elif",
                           mknode("cond",$2,NULL),
                           mknode("then",$4,$5)); }
        ;
...
/* ---------- WHILE ------------------------------------------------------*/
while_state
        : WHILE expression ':' alt_body   /* stmt or block */
              { $$ = mknode("while",$2,$4); }
        ;

/* ---------- DO-WHILE ---------------------------------------------------*/
do_while_state
        : DO ':' alt_body WHILE expression ';'
              { $$ = mknode("do_while",$3,
...
/* ---------- FOR --------------------------------------------------------*/
for_state
        /* with optional local–var section */
        : FOR for_h ':' alt_body
              { $$ = mknode("for",$2,$4); }
        | FOR for_h ':' var alt_body
              { $$ = mknode("for",$2,
                           mknode("block",$5,$4)); }
        ;
...
expression :
    ...
    | TRUE                          { $$ = mknode("BOOL", mknode("TRUE", NULL, NULL), NULL); }
    | FALSE                         { $$ = mknode("BOOL", mknode("FALSE", NULL, NULL), NULL); }
    | LENGTH IDENT LENGTH        { $$ = mknode("length", mknode($2, NULL, NULL), NULL); }
