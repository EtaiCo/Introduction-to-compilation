...
%{
    ...
    /* ─── numeric function scopes ─────────────────────────────────────────── */
static int  nextFuncID  = 1;          /* 0 = file level                    */
static int  curFuncID   = 0;          /* where we are right now            */

static int  blockDepth  = 0;          /* BEGIN…END depth inside cur func   */

/* a  simple stack so we can nest functions safely                        */
static int  funcStack[64];
static int  funcTop = -1;             /* top element in funcStack          */

static void pushFuncScope(void)            { funcStack[++funcTop] = curFuncID; }
static void popFuncScope (void)            { curFuncID = funcStack[funcTop--]; }
static int  newFuncScope(void)             { return nextFuncID++; }
    ...
    typedef struct Symbol {
        ...
        struct Symbol* next;
        int funcID;          /* which function owns the name  */
        int blockDepth;
    } Symbol;
    ...
    static Symbol* lookupFunc(const char* name);
...
/* ---------- helpers that hold *no* semantic value --------------------- */
func_scope_start  /* after we have the IDENT                             */
        : %empty
          {
              pushFuncScope();               /* save parent              */
              curFuncID  = newFuncScope();   /* allocate fresh number    */
              blockDepth = 0;                /* reset for this function  */
          }
        ;

func_scope_end
        : %empty  { popFuncScope(); }        /* restore parent function  */
        ;

BEGIN_MARK  : %empty { ++blockDepth; };
END_MARK    : %empty { --blockDepth; };


function :
    DEF IDENT '(' params ')' ':' RETURNS type func_scope_start var scope_marker T_BEGIN BEGIN_MARK statements END_MARK END func_scope_end
    {
...
int insertSymbol(const char* name, SymbolType t, const char* ret,
                 int paramCnt, char* paramTypes[])
{
    for (Symbol* s = symbolTable; s; s = s->next)
        if (!strcmp(s->name,name) &&
            s->funcID     == curFuncID &&
            s->blockDepth == blockDepth)          /* same block  */
            return 1;                             /* duplicate   */

    Symbol* n = malloc(sizeof *n);
    n->name = strdup(name);             n->type = t;
    n->returnType = ret ? strdup(ret):NULL;
    n->paramCount = paramCnt;
    for(int i=0;i<paramCnt;i++) n->paramTypes[i]=strdup(paramTypes[i]);
...
/* variable lookup: search *only* inside curFuncID, walk blockDepth down */
static Symbol* lookupVar(const char* name)
{
    for (int d = blockDepth; d >= 0; --d)
        for (Symbol* s=symbolTable; s; s=s->next)
            if (s->type==VAR && s->funcID==curFuncID &&
                s->blockDepth==d && !strcmp(s->name,name))
                return s;
    return NULL;
}

/* function lookup: name is enough (allows shadowing from outer scope)   */
static Symbol* lookupFunc(const char* name)
{
    for (Symbol* s=symbolTable; s; s=s->next)
        if (s->type==FUNC && !strcmp(s->name,name))
            return s;
    return NULL;
}
