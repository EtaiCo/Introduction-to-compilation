...
%{
    ...
    /* === 3AC helpers === */
    typedef struct { char *place; } Frag;
    Frag genCmp(const char *op, Frag a, Frag b);
    Frag handleShortCircuitAnd(Frag a, Frag b);
    Frag handleShortCircuitOr (Frag a, Frag b);
    Frag handleNot(Frag a);
...
%type <frag>    expression exp_list func_call
...
char *newTemp () { char buf[32]; sprintf(buf,"t%d", tempCnt++);  return strdup(buf); }
char *newLabel() { char buf[32]; sprintf(buf,"L%d", labelCnt++); return strdup(buf); }
