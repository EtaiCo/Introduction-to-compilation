...
%{
    ...
    void registerParams(node* plist);
...
void registerParams(node* plist)
{
    ...
    while (p) {
        ...
        if (single && strcmp(single->token, "PARAM") == 0) {
            ...
            char* typ = single->left->token;   // parameter type (e.g., INT, REAL, etc.)

            // Normalize type to lowercase
            for (char* t = typ; *t; ++t) *t = tolower(*t);

            // Insert the parameter as a variable into the symbol table
            if (insertSymbol(id, VAR, typ, 0, "param", NULL)) {
                yyerror("Semantic Error: Duplicate parameter name in function.");
                YYERROR; return 1;
            }
        }
        // Move to the next PARAM node in the PARAMS list
