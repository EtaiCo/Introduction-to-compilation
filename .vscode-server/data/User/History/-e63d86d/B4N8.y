...
%{
    ...
    static const char *currentFunc = NULL;   /* current function name */
...
func_name
    : IDENT        { currentFunc = $1; $$ = $1; }
    ;


function :
    DEF func_name '(' params ')' ':' RETURNS type var scope_marker T_BEGIN statements END
    {
  ...
  | DEF func_name '(' params ')' ':' var scope_marker T_BEGIN statements END
    {
        if (moreThanOneMain($2)) YYABORT;
                              ...
                              strcmp(single->token,"ARRAY_INIT")==0))
               {
                ...
                if (strcmp(single->token,"ARRAY_INIT")==0)
                   ...
                   if (idNode && strcmp(idNode->token,"IDENT")==0 && idNode->left) {
                       ...
                       if (insertSymbol(vname, VAR, $2->token, 0, currentFunc, NULL)) {
...
int insertSymbol(char* name, SymbolType type, char* returnType, int paramCount, const char* scope, char* paramTypes[]) {
    ...
    while (current) {
        if (strcmp(current->name,  name ) == 0 &&
            strcmp(current->scope, scope) == 0)   /* compare scope strings */
            return 1;           /* duplicate in the same function */
...
int registerParams(node* plist) {
    ...
    while (p) {
        ...
        if (single && strcmp(single->token, "PARAM") == 0) {
            ...
            if (insertSymbol(id, VAR, typ, 0, currentFunc, NULL)) {
                return 1;
            }
        }
