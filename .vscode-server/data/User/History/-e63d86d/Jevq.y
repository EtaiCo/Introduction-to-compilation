...
function :
    DEF IDENT '(' params ')' ':' RETURNS type var scope_marker T_BEGIN statements END
    {
        ...
        // Rule: _main_ must not take parameters or return value
        ...
        node* bodyN = mknode("BODY", $9, $12);  // $9 = var declarations, $12 = statements
...
char* inferExprType(node* expr) {
    ...
    if (sym && sym->type == VAR && sym->returnType) {
        printf("inferExprType: IDENT '%s' resolves to '%s'\n", expr->token, sym->returnType);
