...
char* inferExprType(node* expr) {
    ...
    if (strcasecmp(expr->token, "INT") == 0 || isdigit(expr->token[0])) return "int";
    if (strcasecmp(expr->token, '.')) return "real";
    if (strcasecmp(expr->token, "true") == 0 || strcmp(expr->token, "false") == 0) return "bool";
