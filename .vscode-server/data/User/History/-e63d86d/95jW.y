...
param :
    PAR type ':' IDENT
    {
        ...
        char* paramType = strdup($2->token);
        for (char* p = paramType; *p; ++p) *p = tolower(*p);
        if (insertSymbol($4, VAR, paramType, 0, "param", NULL)) {
            yyerror("Semantic Error: Parameter name already declared in scope.");
            YYABORT;
        }
