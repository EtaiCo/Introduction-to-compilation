...
%{
    ...
    static int nextFuncID   = 1;   /* 0 is for the file itself        */
    static int currentFunc  = 0;   /* we start outside any function   */
...
function :
    DEF IDENT '(' params ')' ':' RETURNS type scope_marker var T_BEGIN statements END
    {
  ...
  | DEF IDENT '(' params ')' ':' scope_marker var T_BEGIN statements END
    {
        if (moreThanOneMain($2)) YYABORT;
...
/* opens a new scope for every BEGIN … END block */
block_scope_start
        : %empty   { pushScope(); }
        ;

bl_state :
    block_scope_start T_BEGIN statements END
        { popScope(); $$ = mknode("block", $3, NULL); }
  | block_scope_start var T_BEGIN statements END
        { popScope(); $$ = mknode("block", $4, $2); }
...
func_call_state :
  ...
  {
      /* ----- identical to the first part of assign_state ----- */
      ...
      char* rhsType = inferExprType($3);   /* ← returns "bool" for call foo() */
