...
/* === TEMP-SIZE TRACKING ============================================== */
static int tempBytesInFunc = 0;          /* reset at top of every genFunction */

/* byte-size of the value produced by expression e */
static int resultSize(node *e)
{
    const char *t = inferExprType(e);    /* "int", "real", "intptr", … */
    if (!t)             return 4;
    if (!strcasecmp(t,"real")   ||
        !strcasecmp(t,"realptr")||
        !strcasecmp(t,"string"))        /* we store strings by pointer */
        return 8;                       /* 8-byte values */
    return 4;                           /* everything else → 4 bytes  */
}

/* create a fresh temp, add its size to tempBytesInFunc and return its name */
static char *makeTempFor(node *e)
{
    tempBytesInFunc += resultSize(e);    /* <-- accumulate */
...
static void genStmt(node *s)
{
    ...
    genStmt(s->right->left);          /* then-block */
    emit("goto %s", Lend);
    emit("%s:", Lelse);
    genStmt(s->right->right->left);   /* else-block (else->left) */
    emit("%s:", Lend);
    return;
    }
...
static void genFunction(node *f)
{
    ...
    tempBytesInFunc = 0;           /* reset for this function */
    ...
    /* frame size = locals + all temps we just counted */
    ...
    while (d) {
        ...
        const char *typ = single->left->token;     /* INT / REAL … */

        node *idcur = single->right;               /* first VAR_DECL / ... */
        while (idcur) {
            localsBytes += sizeofType(typ);
            idcur = (!strcmp(idcur->token,"ID_LIST")) ? idcur->right : NULL;
        }
